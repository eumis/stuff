...
function M.remap()
    ...
    vim.keymap.set('n', '<space>ff', telescope_builtin.find_files, silent_opts)
    vim.keymap.set('n', '<space>fh', function() telescope_builtin.find_files({ hidden = true, no_ignore = true }) end, silent_opts)
    vim.keymap.set('n', '<space>fs', telescope_builtin.live_grep, silent_opts)
    ...
    vim.keymap.set('n', '<space>ft', telescope_builtin.help_tags, silent_opts)
    vim.keymap.set('n', '<space>fm', telescope_builtin.marks, silent_opts)
    vim.keymap.set('n', '<space>fb', telescope_builtin.buffers, silent_opts)

    local harpoon_ui = require("harpoon.ui")
    vim.keymap.set('n', '<C-f>', harpoon_ui.toggle_quick_menu, silent_opts)
    vim.keymap.set('n', '<C-n>', harpoon_ui.nav_next, silent_opts)
    vim.keymap.set('n', '<C-p>', harpoon_ui.nav_prev, silent_opts)
    local harpoon_mark = require("harpoon.mark")
    vim.keymap.set('n', '<C-m>', harpoon_mark.add_file, silent_opts)
    vim.keymap.set('n', '<C-c>', harpoon_mark.clear_all, silent_opts)

    -- snippets
    local ls = require("luasnip")
    vim.keymap.set({"i", "s"}, "<C-n>", function() ls.jump( 1) end, silent_opts)
    vim.keymap.set({"i", "s"}, "<C-p>", function() ls.jump(-1) end, silent_opts)
end
